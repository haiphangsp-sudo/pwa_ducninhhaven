      ...
      for (const [variantKey, variant] of Object.entries(product.variants || {})) {
        if (variant.id === id) {
          return {
            ...
            price: new Intl.NumberFormat('vi-VN', { style: 'currency', currency: 'VND' }).format(variant.price),
...
export function getCartExtended(state) {
  ...
  return {
    ...
    totalPrice: new Intl.NumberFormat('vi-VN', { style: 'currency', currency: 'VND' }).format(variant.price),
    totalQty:totalQ > 1
